...
```{r damageBranchExc, exercise=TRUE, exercise.setup="femBranch"}

```

```{r damageBranchDamExc-hint-1}
...
```{r damageBranchDamExc-hint-2}
damage <- mutate_branch( damage_orig = NDAM,
                         # create damage_log
  )
```

```{r damageBranchDamExc-solution}
damage <- mutate_branch( damage_orig = NDAM, 
                        damage_log = log(NDAM))
```
...
```{r treeAll, exercise=TRUE, exercise.eval=TRUE, exercise.setup="familyBranch"}
...
```{r treeModDist, exercise=TRUE, exercise.eval=TRUE, exercise.setup="familyBranch"}
...
```{r treeFemDam, include=FALSE, exercise.setup="familyBranch"}
...
```{r treeFemDamExc-solution, exercise=TRUE, exercise.setup="familyBranch"}
multiverse_tree(hurricane_mv, label = TRUE, label_size = 4, label_angle = 45, branches = c("femininity", "damage"))
